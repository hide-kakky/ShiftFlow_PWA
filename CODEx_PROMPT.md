...
- すべての回答で「git commit」用のコマンド例とコミットメッセージ案（日本語）を提示すること。
...
- フロントのファイルを一行でも変更した場合、`APP_VERSION` を必ずインクリメントし、回答にその新しい値を明記すること。
...
- app-config.js にも同様のバージョン管理しているコードがあるので、そちらの更新もする。
- 更新の度合いによって、変更する X.Y.Z は判断する。
- Z の値が 30 以上になった場合は、y に繰り上げて、z は０に戻す。
